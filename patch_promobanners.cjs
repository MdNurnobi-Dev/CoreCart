const fs = require('fs');
const file = 'src/components/PromoBanners.tsx';
let code = fs.readFileSync(file, 'utf8');

// The PromoBanners renders exactly 2 banners typically (or maps over the array).
// Let's rewrite the render part to map over activeBanners.
code = code.replace(
  /return \(\n    <section[\s\S]*<\/section>\n  \);/,
  `
